...
bool Scena::wybierz_drona(int numer_drona, PzG::LaczeDoGNUPlota &Lacze){
  ...
  std::cin >> Kat;
  std::cout << std::endl<< "Podaj odleglosc lotu drona: ";
  std::cin >> odleglosc;
  std::cout << std::endl;
...
bool Scena::Zapis_drona(unsigned int numer_drona, vector3d &polozenie){
  ...
  if (!Drony[numer_drona].owektor(numer_drona)) return false;
    return true;
}
...
void Scena::Dodaj_Przeszkody(PzG::LaczeDoGNUPlota &Lacze){
  ...
  std::cout << "3. Wyzyna ze stokiem" << std::endl<< std::endl;
  std::cin >> wybor;
  std::cout << "Podaj współrzędne OX, OY, OZ przeszkody:" << std::endl<< std::endl;
  std::cin >> pozycja;
  std::cout << "Podaj skale przeszkody OX, OY, OZ :" << std::endl<< std::endl;
  std::cin >> skala;
...
void Scena::Usun_Przeszkody(PzG::LaczeDoGNUPlota &Lacze){
  ...
  if (wybor <= Przeszkody.size())
  {
    ...
    std::cout << "Tam nic nie ma!" << std::endl << std::endl;
}
